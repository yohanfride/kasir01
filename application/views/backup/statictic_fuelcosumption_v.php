...
<?php include("header.php") ?>
			<div class="page-inner">
					<div class="page-header">
						<h4 class="page-title">Fuel Cosumption Behaviour</h4>
						<ul class="breadcrumbs">
							...
							<li class="nav-item">
								<a href="#">Fuel Cosumption Behaviour</a>
							</li>
						</ul>
					</div>
					<div class="row">
							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="card-title">Maps - Fuel Cosumption Behaviour</h4>
									</div>
								</div>
								<div class="card-body" style="overflow:hidden;">
                                    <iframe
                                        src="<?= $metabase['maps1']->link; ?>?now=1"
                                        frameborder="0"
                                        allowtransparency 
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#btnSrc").click(function(){
            ...
            var url = "";
            if( tipe == 1){
                url = "<?= $metabase['maps1']->link; ?>?now=1";
            } else if( tipe == 2){
                url = "<?= $metabase['maps1']->link; ?>?week=1";
            } else if( tipe == 3){
                url = "<?= $metabase['maps1']->link; ?>?month=1";
            } else if( tipe == 4){
                var date =  $("#inputDate").val();
                url = "<?= $metabase['maps2']->link; ?>?date="+date;
            } else if( tipe == 5){
                var datestr =  $("#inputStart").val();
                var dateend =  $("#inputEnd").val();
                url = "<?= $metabase['maps2']->link; ?>?start="+datestr+"&end="+dateend;
            }
            $('#iframe').attr('src', url);
        });
    });
